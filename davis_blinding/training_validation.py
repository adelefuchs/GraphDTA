...
datasets = [["davis_b2", "kiba", "pharos"][int(sys.argv[1])]]
    ...
    if (not os.path.isfile(processed_data_file_train)) or (
        ...
        for epoch in range(NUM_EPOCHS):
            ...
            if val < best_mse:
                ...
                prediction_file_name = f"predictions_{model_st}_{dataset}.csv"
                df = pd.DataFrame({"True_Label": G, "Predicted_Value": P})
                df.to_csv(prediction_file_name, index=False)
                print(f"Predictions saved to {prediction_file_name}")
                best_test_mse = ret[1]
                best_test_ci = ret[-1]
                print(
